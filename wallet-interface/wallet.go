...
type walletMustManager interface {
	...
	// CurrencyCode returns the currency code this wallet implements. For example, "BCH".
	// When running on testnet a `T` should be prepended. For example "TBCH".
	CurrencyCode() string
	...
	// involving them.
	AddWatchedAddress(addr bchutil.Address) error
	RemoveWatchedAddress(addr bchutil.Address) error
	...
	AddTransactionListener(bool, func(TransactionCallback)) int
	RemoveTransactionListener(int) error

	AddBlockListener(func(BlockCallback)) int
	RemoveBlockListener(int) error
	...
	CurrentAddress(purpose KeyPurpose) bchutil.Address
	...
	// was for.
	NewAddress(purpose KeyPurpose) bchutil.Address
...
type walletMustBanker interface {
	...
	Spend(amount int64, addr bchutil.Address, feeLevel FeeLevel, referenceID string, spendAll bool) (*chainhash.Hash, error)
	...
	SweepAddress(ins []TransactionInput, address *bchutil.Address, key *hd.ExtendedKey, redeemScript *[]byte, feeLevel FeeLevel) (*chainhash.Hash, error)
}
